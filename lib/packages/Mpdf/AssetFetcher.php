...
use Dinamiko\DKPDFG\Vendor\Mpdf\File\LocalContentLoaderInterface;
use Dinamiko\DKPDFG\Vendor\Mpdf\File\StreamWrapperChecker;
use Dinamiko\DKPDFG\Vendor\Mpdf\Http\ClientInterface;
use Dinamiko\DKPDFG\Vendor\Mpdf\Log\Context as LogContext;
use Dinamiko\DKPDFG\Vendor\Mpdf\PsrHttpMessageShim\Request;
use Dinamiko\DKPDFG\Vendor\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use Dinamiko\DKPDFG\Vendor\Psr\Log\LoggerInterface;

class AssetFetcher implements \Dinamiko\DKPDFG\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \Dinamiko\DKPDFG\Vendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \Dinamiko\DKPDFG\Vendor\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if ($this->mpdf->debug) {
				throw new \Dinamiko\DKPDFG\Vendor\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
