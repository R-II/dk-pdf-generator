 ...
 */
class S25 extends \Dinamiko\DKPDFG\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDFG\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $checksum)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$digit])) {
				// invalid character
				throw new \Dinamiko\DKPDFG\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in S25 barcode value "%s"', $digit, $code));
			}
			$seq .= $chr[$digit];
		}
